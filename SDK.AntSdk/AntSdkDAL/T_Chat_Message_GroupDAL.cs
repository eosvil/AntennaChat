...
{
    public class T_Chat_Message_GroupDAL : IDAL<SDK.AntSdk.AntModels.AntSdkChatMsg.ChatBase>
    {
        ...
        {
            var selectStr =
                $"select  * from t_chat_message_group  where chatIndex<>'{chatIndex}' and sendtime<>'' and sendtime not null  and sessionId='{sessionId}' order by rowid desc limit 0,1";
        ...
        public IList<SDK.AntSdk.AntModels.AntSdkChatMsg.ChatBase> GetList(SDK.AntSdk.AntModels.AntSdkChatMsg.ChatBase model)
        {
            throw new NotImplementedException();
        }

        public IList<SDK.AntSdk.AntModels.AntSdkChatMsg.ChatBase> GetListPage(string session_id, string userId, string companyCode, int index, int pageSize)
        {
            throw new NotImplementedException();
        }
        /// <summary>
        ...
        {
            if (model.sourceContent != null) model.sourceContent = model.sourceContent.Replace("'", "''");
            string insertStr = "insert into T_Chat_Message_Group(MTP,CHATINDEX,CONTENT,MESSAGEID,SENDTIME,SENDUSERID,SESSIONID,TARGETID,SENDORRECEIVE,SENDSUCESSORFAIL,SPARE1) values ('" + (int)model.MsgType + "','" + model.chatIndex + "','" + model.sourceContent + "','" + model.messageId + "','" + model.sendTime + "','" + model.sendUserId + "','" + model.sessionId + "','" + model.targetId + "','" + model.SENDORRECEIVE + "','" + 1 + "','" + model.VoteOrActivityID + "')";
        ...
        /// <summary>
        ///根据MessageID删除群组对应的消息记录
        /// </summary>
        /// <param name="session_id"></param>
        /// <param name="companyCode"></param>
        /// <param name="userId"></param>
        /// <param name="messageId"></param>
        public int DeleteGroupMsgByMessageId(string companyCode, string userId,
            string messageId)
        {
            string updateStr = "delete from t_chat_message_group where messageid='" +
                               messageId + "';";
            string dbPath = $@"{AntSdkService.SqliteLocalDbPath}{userId}\{userId}.db";
            if (File.Exists(dbPath))
            {
        ...
        /// <param name="messageid"></param>
        /// <returns></returns>
        public bool GroupMsgExit(string messageid)
        {
            string exitStr = "select count(messageid) from t_chat_message_group  where  messageid='" + messageid + "'";
        ...
        {
            string updateStr = "update t_chat_message_group set sendsucessorfail='1' where messageid='" + messageid + "'";
            string dbPath = $@"{AntSdkService.SqliteLocalDbPath}{userId}\{userId}.db";
            if (File.Exists(dbPath))
            {
                return AntSdkSqliteHelper.ExecuteNonQuery(updateStr, companyCode, userId);
        ...
        {
            string updateStr = "update t_chat_message_group set CONTENT='" + content + "' where messageid='" + messageid + "'";
            string dbPath = $@"{AntSdkService.SqliteLocalDbPath}{userId}\{userId}.db";
            if (File.Exists(dbPath))
            {
                return AntSdkSqliteHelper.ExecuteNonQuery(updateStr, companyCode, userId);
        ...
        {
            string updateStr = "update t_chat_message_group set sendsucessorfail='1',chatindex='" + chatindex + "' where messageid='" + messageid + "'";
            string dbPath = $@"{AntSdkService.SqliteLocalDbPath}{userId}\{userId}.db";
            if (File.Exists(dbPath))
            {
                return AntSdkSqliteHelper.ExecuteNonQuery(updateStr, companyCode, userId);
        ...
        {
            string selectStr = "select sendtime,cast(sendsucessorfail as varchar) as sendsucessorfail, messageid from t_chat_message_group where messageid='" + messageid + "'";
            string dbPath = $@"{AntSdkService.SqliteLocalDbPath}{AntSdkService.AntSdkLoginOutput.userId}\{AntSdkService.AntSdkLoginOutput.userId}.db";
            if (File.Exists(dbPath))
            {
                return AntSdkSqliteHelper.ExecuteDataTable(selectStr, dbPath);
        ...
        {
            string updateStr = "update t_chat_message_group set voiceread='1' where messageid='" + messageid + "'";
            string dbPath = $@"{AntSdkService.SqliteLocalDbPath}{AntSdkService.AntSdkLoginOutput.userId}\{AntSdkService.AntSdkLoginOutput.userId}.db";
            if (File.Exists(dbPath))
            {
