...
{
    internal delegate void PublicationReceivedHandler(string topic, byte[] payload);
    internal class MQTTClient : DisposableBase
    {
        ...
        private int DefaultTimeout = 1200000;
        ...
        private MsConnectionState m_state;
        ...
        public string SSLTargetHost { get; private set; } = string.Empty;
        public string BrokerHostName { get; private set; } = string.Empty;
        ...
        public MQTTClient(string brokerHostName, int brokerPort, bool useSSL, string sslTargetHost)
        {
            ...
            ConnectionState = MsConnectionState.Disconnected;
        ...
        public MsConnectionState ConnectionState
        {
            get { return m_state; }
            private set
            {
                ...
                else
                {
                    ...
                    {
                        case MsConnectionState.Disconnected:
        ...
        private bool Connect(string hostName, int port)
        {
            ...
            ConnectionState = MsConnectionState.Connecting;
        ...
        {
            get { return ConnectionState == MsConnectionState.Connected; }
            private set
            {
                if (value)
                {
                    ConnectionState = MsConnectionState.Connected;
                ...
                else
                {
                    ...
                    {
                        TracingDebug("Disconnected Event Is Null");
                    }
        ...
        private void DoReconnect()
        {
            switch (ConnectionState)
            {
                case MsConnectionState.Connected:
                case MsConnectionState.Connecting:
                    return;
            }
            ...
            if (m_shouldReconnect)
            {
                ...
                try
                {
                    ...
                    if (ConnectionState == MsConnectionState.Connecting)
                    {
                        ...
                        if (autoResetConnEvent.WaitOne(5000, false))
                        {
                            ...
                            if (ReconnectedMqtt != null)//需要重新推送$queue/user_online
                            {
        ...
        public void Publish(string topic, byte[] data, QoS qos, bool retain)
        {
            ...
            else
            {
                ...
                TracingDebug("Send Publish: topic=" + topic);
            }
        ...
        private void TracingDebug(string message)
        {
            if (TracingEnabled)
            {
                ...
                LogHelper.WriteInfo($"{DateTime.Now.ToString("HH:mm:ss")},{message}");
            }
        }
    }
}
