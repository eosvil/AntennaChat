...
{
    internal class SubscriptionCollection : IEnumerable<Subscription>
    {
        public Dictionary<string, Subscription> m_subscriptions = new Dictionary<string, Subscription>();

        //由一次只能（取消）订阅一个主题修改为可一次（取消）订阅多个主题 Modify by 赵雪峰 20160910
        ...
        //    return s;
        //}
        //由一次只能订阅一个主题，修改为一次可订阅多个主题 Modify by 赵雪峰 20160910
        public void Add(Dictionary<string, QoS> subscriptions)
        {
            ...
            if (SubscriptionAdded != null&& subscribeList.Count >0)
            {
                SubscriptionAdded(this, new GenericEventArgs<List<Subscription>>(subscribeList));
            }
        }
