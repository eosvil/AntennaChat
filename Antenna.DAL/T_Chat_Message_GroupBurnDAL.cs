...
{
    public class T_Chat_Message_GroupBurnDAL : IDAL<ChatMsgReceive>
    {
        ...
        {
            string selectStr = "select * from (select  * from t_chat_message_groupburn where sessionid='" + session_id + "' order by sendtime desc limit " + index + "," + pageSize + ") order by sendtime; ";
            string dbPath = publicMethod.localDataPath() + companyCode + "\\" + userId + "\\" + userId + ".db";
            if (File.Exists(dbPath))
            {
                LogHelper.WriteDebug("[T_Chat_MessageDAL-GetDataTable:]:" + "sql:" + selectStr + "数据库:" + dbPath);
                return SqliteHelper.ExecuteDataTable(selectStr, dbPath);
            }
            else
            {
                LogHelper.WriteDebug("[T_Chat_MessageDAL-GetDataTable:]:没有数据库");
                return null;
            }
        }
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        public int ReceiveInsertBurn(ChatMsgReceive model)
        {
            if (model.content != null) model.content = model.content.Replace("'", "''");
            string insertStr = "insert into t_chat_message_groupburn(MTP,CHATINDEX,CONTENT,MESSAGEID,SENDTIME,SENDUSERID,SESSIONID,TARGETID,SENDORRECEIVE,SENDSUCESSORFAIL) values ('" + model.MTP + "','" + model.chatIndex + "','" + model.content + "','" + model.messageId + "','" + model.sendTime + "','" + model.sendUserId + "','" + model.sessionId + "','" + model.targetId + "','" + model.SENDORRECEIVE + "','" + model.sendsucessorfail + "')";
            return SqliteHelper.ExecuteNonQuery(insertStr, model.companyCode, AntSdkService.AntSdkLoginOutput.userId);
        }
        /// <summary>
        /// 多端数据插入
        ...
        {
            string insertStr = "update t_chat_message_groupburn set chatindex='" + model.chatIndex + "',sendtime='" + model.sendTime + "',SENDSUCESSORFAIL='1' where messageid='" + model.messageId + "'";
            return SqliteHelper.ExecuteNonQuery(insertStr, model.companyCode, model.sendUserId);
        }
        ...
        {
            string selectStr = "select * from (select  * from t_chat_message_groupburn where sessionid='" + session_id + "' and sendtime between '" + startTimestamp + "' and '" + endTimestamp + "' order by sendtime asc limit 0," + pageSize + ") order by sendtime; ";
        ...
        {
            string selectStr = "select count(*) from t_chat_message_groupburn where sessionid='" + session_id + "' and chatindex<'" + start_index + "'";
            string dbPath = publicMethod.localDataPath() + GlobalVariable.CompanyCode + "\\" + AntSdkService.AntSdkCurrentUserInfo.userId + "\\" + AntSdkService.AntSdkCurrentUserInfo.userId + ".db";
            if (File.Exists(dbPath))
            {
                return Convert.ToInt32(SqliteHelper.ExecuteScalar(selectStr, dbPath));
        ...
        {
            string selectStr = "select count(*) from t_chat_message_groupburn where sessionid='" + session_id + "' and chatindex>'" + end_index + "'";
            string dbPath = publicMethod.localDataPath() + GlobalVariable.CompanyCode + "\\" + AntSdkService.AntSdkCurrentUserInfo.userId + "\\" + AntSdkService.AntSdkCurrentUserInfo.userId + ".db";
            if (File.Exists(dbPath))
            {
                return Convert.ToInt32(SqliteHelper.ExecuteScalar(selectStr, dbPath));
        ...
        {
            string selectStr = "select * from (select  * from t_chat_message_groupburn where sessionid='" + session_id + "' and chatindex< '" + start_index + "' order by chatindex desc limit 0," + pageSize + ") order by sendtime asc;";
            string dbPath = publicMethod.localDataPath() + GlobalVariable.CompanyCode + "\\" + AntSdkService.AntSdkCurrentUserInfo.userId + "\\" + AntSdkService.AntSdkCurrentUserInfo.userId + ".db";
            if (File.Exists(dbPath))
            {
                return SqliteHelper.ExecuteDataTable(selectStr, dbPath);
        ...
        {
            string selectStr = "select * from (select  * from t_chat_message_groupburn where sessionid='" + session_id + "' and chatindex> '" + end_index + "' order by chatindex asc limit 0," + pageSize + ") order by sendtime asc;";
            string dbPath = publicMethod.localDataPath() + GlobalVariable.CompanyCode + "\\" + AntSdkService.AntSdkCurrentUserInfo.userId + "\\" + AntSdkService.AntSdkCurrentUserInfo.userId + ".db";
            if (File.Exists(dbPath))
            {
                return SqliteHelper.ExecuteDataTable(selectStr, dbPath);
        ...
        {
            string selectStr = "select * from t_chat_message_groupburn where sessionid='" + session_id + "' and sendtime!='' and cast(chatindex as int) <=" + startChatIndex + " order by sendtime desc limit '" + 0 + "',10";
            string dbPath = publicMethod.localDataPath() + companyCode + "\\" + userId + "\\" + userId + ".db";
            if (File.Exists(dbPath))
            {
                return SqliteHelper.ExecuteDataTable(selectStr, dbPath);
        ...
        {
            string selectStr = "select * from t_chat_message_groupburn where sessionid='" + session_id + "' and cast(chatindex as int) <" + startChatIndex + " order by sendtime desc limit '" + 0 + "',10";
            string dbPath = publicMethod.localDataPath() + companyCode + "\\" + userId + "\\" + userId + ".db";
            if (File.Exists(dbPath))
            {
                return SqliteHelper.ExecuteDataTable(selectStr, dbPath);
        ...
        {
            string selectStr = "select max(cast(chatindex as int)) as chatindex from t_chat_message_groupburn where sessionid='" + session_id + "' and sendtime!=''";
            string dbPath = publicMethod.localDataPath() + companyCode + "\\" + userId + "\\" + userId + ".db";
            if (File.Exists(dbPath))
            {
                var result = SqliteHelper.ExecuteScalar(selectStr, dbPath);
                LogHelper.WriteDebug("[t_chat_message_groupburn]:" + "sql:" + selectStr + "result:" + result + "数据库:" + dbPath);
                if (result == null)
                {
                    return "0";
                }
            ...
            {
                LogHelper.WriteDebug("[t_chat_message_groupburn]:没有数据库");
                return "0";
            }
        }
        /// <summary>
        ...
        {
            string selectStr = "select chatindex from t_chat_message_groupburn where sessionid='" + session_id + "' and sendtime!='' order by chatindex desc limit " + unreadCount + ",1;";
            string dbPath = publicMethod.localDataPath() + companyCode + "\\" + userId + "\\" + userId + ".db";
            if (File.Exists(dbPath))
            {
                var result = SqliteHelper.ExecuteScalar(selectStr, dbPath);
        ...
        {
            string updateStr = "update t_chat_message_groupburn set uploadordownpath='" + path + "' where sessionid='" + session_id + "' and chatindex='" + chatIndex + "'";
            string dbPath = publicMethod.localDataPath() + companyCode + "\\" + userId + "\\" + userId + ".db";
            if (File.Exists(dbPath))
            {
        ...
        public int DeleteBurnData(string session_id, string companyCode, string userId)
        {
            ...
            if (File.Exists(dbPath))
            {
                return SqliteHelper.ExecuteNonQuery(deleteStr, companyCode, userId);
            }
            else
            {
                return 0;
        ...
        {
            string deleteStr = "delete from t_chat_message_groupburn where sessionid='" + session_id + "' and cast(chatindex as int)<="+chatIndex+"";
            string dbPath = publicMethod.localDataPath() + companyCode + "\\" + userId + "\\" + userId + ".db";
            if (File.Exists(dbPath))
            {
                return SqliteHelper.ExecuteNonQuery(deleteStr, companyCode, userId);
