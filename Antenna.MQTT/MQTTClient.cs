...
{
    //public delegate void PublicationReceivedHandler(string topic, QoS qos, byte[] payload); Modify by 赵雪峰 20160910
    public delegate void PublicationReceivedHandler(string topic, byte[] payload);
                ...
                else
                {
                    switch (ConnectionState)
                    {
                        case MQTT.ConnectionState.Disconnected:
                            {
                                ReconnectedMqtt?.Invoke(null, null);//开始重连
